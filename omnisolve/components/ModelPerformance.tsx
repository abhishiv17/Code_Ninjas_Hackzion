    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className="space-y-6">
          <MetricBar label="Accuracy" value={accuracy} target={92} color="bg-blue-500" glow="rgba(59,130,246,0.6)" active={active} icon={Target} subtext="Correct predictions overall" />
          <MetricBar label="Precision" value={precision} target={89} color="bg-purple-500" glow="rgba(168,85,247,0.6)" active={active} icon={CheckCircle2} subtext="Correct specific fixes" />
          <MetricBar label="Recall" value={recall} target={94} color="bg-green-500" glow="rgba(34,197,94,0.6)" active={active} icon={Activity} subtext="Ability to catch all true issues" />
        </div>
      </div>
    </motion.div>
  );
