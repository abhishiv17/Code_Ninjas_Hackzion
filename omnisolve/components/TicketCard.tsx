  ...
  if (!ticket) {
    return (
      <div className="bg-card border border-white/5 rounded-xl p-6 h-[200px] flex items-center justify-center flex-col gap-3">
        ...
        <p className="text-sm text-gray-500">{t('awaiting_alerts')}</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] font-bold px-2 py-0.5 rounded pl-3 bg-red-500/10 text-red-400 uppercase tracking-widest border border-red-500/20 drop-shadow-[0_0_3px_rgba(239,68,68,0.5)] flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></div>{t(ticket.priority.toLowerCase())} {t('ticket_upper')}</span>
            <span className="text-xs font-mono text-gray-500 bg-white/5 px-2 py-0.5 rounded">{ticket.id}</span>
          </div>
          <h3 className="text-lg font-bold text-gray-100 mt-2 group-hover:text-white transition-colors">{t(ticket.type.toLowerCase())} {t("alert_upper")}</h3>
        </div>
        ...
        <div className="px-2.5 py-1 rounded-md bg-blue-500/10 border border-blue-500/20 flex items-center gap-1.5 shadow-[inset_0_0_10px_rgba(59,130,246,0.1)]">
          ...
          <span className="text-xs font-semibold text-blue-400">{t('ot_config')}</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 relative z-10">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-400 bg-white/5 px-2 py-1 rounded-md border border-red-500/10">
          ...
          <span className="text-red-200">{t('just_now')}</span>
        </div>
      </div>
    </motion.div>
  );
