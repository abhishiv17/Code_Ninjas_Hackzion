...
import { CheckCircle2, ChevronRight, FileText, Sparkles, AlertTriangle, Cpu } from 'lucide-react';
    ...
    return (
      <div className="bg-card/60 backdrop-blur-md border border-blue-500/30 rounded-2xl p-5 flex flex-col items-center justify-center min-h-[300px] shadow-[inset_0_0_30px_rgba(59,130,246,0.1),0_0_20px_rgba(59,130,246,0.15)] relative overflow-hidden">
        ...
        <div className="relative mb-6">
          ...
          <Cpu className="w-10 h-10 text-blue-400 animate-pulse relative z-10" />
        </div>
    ...
    >
      ...
      <div className="bg-gradient-to-r from-blue-600/10 to-purple-600/5 px-5 py-3 border-b border-blue-500/20 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-semibold text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">AI Resolution Plan</h3>
        </div>
        <div className="flex items-center gap-2 bg-blue-500/20 px-2.5 py-1 rounded-md border border-blue-500/30 shadow-[inset_0_0_10px_rgba(59,130,246,0.2)]">
          ...
          <span className="text-xs font-semibold text-blue-100">{analysis.confidence}% Confidence</span>
        </div>
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col gap-4 relative z-10">
        ...
        {analysis.duplicate && (
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-yellow-200">Duplicate Issue Detected</p>
              <p className="text-xs text-yellow-500/90 mt-1 font-medium">
                Matches historical ticket <strong className="text-yellow-300 bg-yellow-500/20 px-1 py-0.5 rounded">{analysis.similar_ticket}</strong>. (Resolved in {analysis.similar_ticket_time}). Optimizing parameters.
              </p>
            </div>
          </motion.div>
        )}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}>
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Plan of Action</h4>
            {analysis.estimated_time && (
              <span className="text-xs bg-white/5 border border-white/10 px-2 py-0.5 rounded text-gray-400">Est: {analysis.estimated_time}</span>
            )}
          </div>
          <div className="bg-black/40 rounded-lg border border-white/5 p-4 space-y-3">
            {(analysis.steps || analysis.solution.split('\n')).map((step, i) => (
              <motion.div 
                key={i} 
